...
def extract_data_from_h5_group(h5_group, normalize=True):
    name = np.array(h5_group.get("name"))
    idxs = np.array(h5_group.get("idxs"))
    ...
    return V_1, V_2, A_1_idx, A_1_values, A_1_shape, A_2_idx, A_2_values,\
           A_2_shape, A_3_idx, A_3_values, A_3_shape, labels, name, idxs


def write_h5_batch_file(file_path, batch_path, vertices_per_batch=10000):
    hf = h5py.File(file_path, 'r')
    batch_counter = 0
    batch_num = 0
    vertex_count = 0

    V_1_batch = V_2_batch = I_1_batch = idxs_batch = names = labels_batch = None
    ...
    for key in keys:
        group = hf.get(key)
        V_1, V_2, A_1_idx, A_1_values, A_1_shape, A_2_idx, A_2_values,\
        A_2_shape, A_3_idx, A_3_values, A_3_shape, labels, name, idxs = extract_data_from_h5_group(group, normalize=True)
        ...
        if vertex_count >= vertices_per_batch:
            ...
            batch = [names, V_1_batch, A_1_batch_idx, A_1_batch_values, A_1_batch_shape, V_2_batch, A_2_batch_idx,
                     A_2_batch_values, A_2_batch_shape, A_3_batch_idx, A_3_batch_values, A_3_batch_shape, I_1_batch,
                     labels_batch, idxs_batch]
            ...
            batch_counter = 0
            batch_num += 1
            V_1_batch = V_2_batch = I_1_batch = names = labels_batch = idxs_batch = None
            A_1_batch_idx = A_1_batch_values = A_1_batch_shape = None
            A_2_batch_idx = A_2_batch_values = A_2_batch_shape = None
            A_3_batch_idx = A_3_batch_values = A_3_batch_shape = None
        ...
        # If limit is not reached add new graph to batch.
            ...
            # If there are graphs in current batch
                ...
                idxs_batch = np.append(idxs_batch, idxs, axis=0)
...
def write_batch_to_file(batch_num, batch, file_path):
    ...
    batch_group.create_dataset("V_2", data=batch[5])
    batch_group.create_dataset("A_2_idx", data=batch[6])
    batch_group.create_dataset("A_2_values", data=batch[7])
    batch_group.create_dataset("A_2_shape", data=batch[8])
    batch_group.create_dataset("A_3_idx", data=batch[9])
    batch_group.create_dataset("A_3_values", data=batch[10])
    batch_group.create_dataset("A_3_shape", data=batch[11])
    batch_group.create_dataset("I_1", data=batch[12])
    batch_group.create_dataset("labels", data=batch[13])
    batch_group.create_dataset("idxs", data=batch[14])
...
if __name__ == "__main__":
    ...
    write_h5_batch_file(read_file_path, batch_h5_path, vertices_per_batch=10000)  