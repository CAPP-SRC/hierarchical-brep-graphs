...
def write_h5_file(h5_path, graph_name, work_faces, work_facets, work_face_edges, work_facet_edges,
                  save_sparse=True):
    hf = h5py.File(h5_path, 'a')
    hier_group = hf.create_group(str(graph_name))
    label = int(str(graph_name).split("-")[0])
    labels = np.array([label])

    V_1, _ = get_face_features(work_faces)
